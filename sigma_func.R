...
sigma_function <- function (S0, r, nsteps, npaths, T, dt, K, discount, sigma_steps) {
  set.seed(1234567890)
  return_list <- numeric(length(sigma_steps))
  avg_optimal_stopping <- numeric(length(sigma_steps))
  count_optimal_stopping <- numeric(length(sigma_steps))
  index <- 1
  ...
  for (sigma in sigma_steps) {
    ...
    return_list[index] <- mean(discounted_cashflows)
