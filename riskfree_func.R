...
riskfree_function <- function (S0, nsteps, npaths, T, dt, K, sigma, discount, r_steps) {
  set.seed(1234567890)
  return_list <- numeric(length(r_steps))
  avg_optimal_stopping <- numeric(length(r_steps))
  count_optimal_stopping <- numeric(length(r_steps))
  index <- 1
