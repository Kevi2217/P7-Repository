...
continuation_matrix[, nsteps + 1] <- payoff[, nsteps + 1]
...
for (t in nsteps:1) {
  ...
  # If there are any in-the-money paths
  if (sum(in_the_money) > 0) {
    # Extract the stock prices and future payoffs for in-the-money paths
    X <- S[in_the_money, t]
    Y <- payoff[in_the_money, t + 1] * discount # Discounted future payoff
    ...
    L2 <- exp(-X/2)*(1-2*X+X^2/2)
    L3 <- exp(-X/2)*(1-3*X+(3*X^2/2)-(X^3/6))
    # Use regression to estimate continuation values
    continuation_matrix[in_the_money, t] <- predict(lm(Y ~ L0 + L1 + L2 + L3),
                                                    newdata = data.frame(
                                                      L0 = exp(-X/2),
                                                      L1 = exp(-X/2) * (1 - X),
                                                      L2 = exp(-X/2) * (1 - 2*X + X^2 / 2),
...
compute_dual_price <- function(npaths, continuation_values, payoff_matrix, discount, dt) {
  
  dual_values <- numeric(npaths) # Store dual prices for each path
  
  for (i in 1:npaths) {
    martingale <- 0 # Initialize martingale correction term
    dual_value <- payoff_matrix[i, nsteps + 1] # Start with the terminal payoff (no continuation beyond final time)
    
    for (t in nsteps:1) { # Step backward in time
      # Always discount the martingale by one time step
      martingale <- martingale * discount
      # Determine if the option is in the money at time t
      in_the_money <- (payoff_matrix[i, t] > 0)
      
      if (in_the_money) {
        # Update martingale correction: add discounted continuation value
        martingale <- martingale + payoff_matrix[i, t]
        
        # Compute the adjusted value considering the continuation value
        # Dual price is max of holding (adjusted by martingale) vs early exercise
        dual_value <- max(dual_value, martingale - continuation_values[i, t])
      }
    }
  ...
  # Return the average dual price across all paths
  return(mean(dual_values))
}
# Compute dual price
dual_estimate <- compute_dual_price(npaths, continuation_matrix, payoff, discount, dt)
# cat("Bermudan Call Option Price (Dual):", dual_estimate, "\n")

# Comparing primal and dual estimates
cat("Primal Estimate:", mean(disc_cashflow), "\n")
...
# Plot 1
S_plot <- as.data.frame(S[6969:6988, ]) %>% 
...
geom_histogram(unlist(apply(cashflow, 1, function(row) {which(row > 0)})) - 1,
     breaks = seq(0.5, ncol(cashflow) - 1 + 0.5, by = 1), 
     main = "Histogram of Stopping Times", 
     xlab = "Time", 
     ylab = "Frequency",
     col = "lightblue")
...
for (i in (nsteps+1):1) {
  for (j in 1:i) {
    ...
    cat("Node (i=", i, ", j=", j, "): Payoff=", payoff_tree[j, i],
        ", Continuation=", continuation_value, ", Stop=", stop_tree[j, i], "\n")
  }
}
...
cat("Fair value of the option (binomial model):", round(option_price_tree, 4), "\n")
