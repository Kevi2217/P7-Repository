...
price_se_convergence <- numeric(length(convergence_step))
index <- 1
...
price_convergence_list[index] <- mean(disc_cashflow)
...
price_se_convergence[index] <- sd(disc_cashflow)
index <- index + 1   
}
return(list(price_convergence_list, avg_optinal_stopping, count_optimal_stopping, price_se_convergence))
}